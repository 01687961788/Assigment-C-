    ...
    public partial class fAdmin : Form
    {
        ...
        private bool checkProduct()
        {
            ...
            if (isAddNewProduct)
            {
                ...
                if (cate.Equals("Coffe") && !Regex.IsMatch(check, "^[C][F][0-9]{2}$"))
                {
                    MessageBox.Show("Danh mục là "+cate+" thì\n Mã sản phẩm phải theo đinh dạng [CF00]", "Thông báo");
                    txtFoodID.Clear();
                    txtFoodID.Focus();
                    return false;
                }
                if (cate.Equals("Tea") && !Regex.IsMatch(check, "^[T][E][0-9]{2}$"))
                {
                    MessageBox.Show("Danh mục là " + cate + " thì\n Mã sản phẩm phải theo đinh dạng [TE00]", "Thông báo");
                    txtFoodID.Clear();
                    txtFoodID.Focus();
                    return false;
                }
                if (cate.Equals("Ice Blender") && !Regex.IsMatch(check, "^[I][B][0-9]{2}$"))
                {
                    MessageBox.Show("Danh mục là " + cate + " thì\n Mã sản phẩm phải theo đinh dạng [IB00]", "Thông báo");
                    txtFoodID.Clear();
                    txtFoodID.Focus();
                    return false;
                }
                if (cate.Equals("Cake") && !Regex.IsMatch(check, "^[C][K][0-9]{2}$"))
                {
                    MessageBox.Show("Danh mục là " + cate + " thì\n Mã sản phẩm phải theo đinh dạng [CK00]", "Thông báo");
                    txtFoodID.Clear();
                    txtFoodID.Focus();
                    return false;
                }
                if (cate.Equals("Sandwitch") && !Regex.IsMatch(check, "^[S][W][0-9]{2}$"))
                {
                    MessageBox.Show("Danh mục là " + cate + " thì\n Mã sản phẩm phải theo đinh dạng [SW00]", "Thông báo");
                    txtFoodID.Clear();
                    txtFoodID.Focus();
                    return false;
                }
        ...
        {
            /*accList = AccountDAO.Instance.getListAccount();
        ...
        {
            if (AccountDAO.Instance.updateAccount(username, fullname, isAdmin))
            {
        ...
        /*txtAccountID.ReadOnly = true;
            txtAccountID.Text = dtgvAccount.CurrentRow.Cells["Tên tài khoản"].Value.ToString();
            txtAccountName.Text = dtgvAccount.CurrentRow.Cells["Họ tên"].Value.ToString();
            bool isAdmin = dtgvAccount.CurrentRow.Cells["Quyền Admin"].Value.ToString() == "True";
